import React from 'react';
import { Link } from 'react-router-dom';
import Chart from '../../components/Chart/Chart';
import "./product.css"
import {productData} from "../../data"
import CloudUploadIcon from '@material-ui/icons/CloudUpload';

const Product = () => {
    return (
        <div className="product">
            <div className="productTopSection">
                <h1 className="productTitle">product</h1>
                <Link to="/newproduct">
                    <button className="productCreateButton">create</button>
                </Link>
            </div>
            <div className="productTop">
                <div className="productLeft">
                    <Chart  data={productData} dataKey="sales"  title="sales performance"/>
                </div>
                <div className="productRight"> 
                      <div className="productRiTopItem">
                          <img className="productTopImage" src="data:image/jpeg;base64," alt=""/>
                          <div className="productTopName">apple airpod</div>
                      </div>
                  <div className="productRiBottom">
                      <div className="productBottomItem">
                          <span className="prductInfoKey"></span>
                          <span className="prductInfoValue"></span>
                      </div>
                      <div className="productBottomItem">
                          <span className="prductInfoKey">id:</span>
                          <span className="prductInfoValue">232</span>
                      </div>
                      <div className="productBottomItem">
                          <span className="prductInfoKey">sales:</span>
                          <span className="prductInfoValue">2321</span>
                      </div>
                      <div className="productBottomItem">
                          <span className="prductInfoKey">active:</span>
                          <span className="prductInfoValue">yes</span>
                      </div>
                      <div className="productBottomItem">
                          <span className="prductInfoKey">in stock:</span>
                          <span className="prductInfoValue">no</span>
                      </div>
                  </div>
                </div>
            </div>

            <div className="productBottom">
                
            <div className="productUpdate">

                  <form className="productUpdateForm">
                      
                    <div className="productUpdateLeft">
                  
                    <h4 className="productUpdatTitle">product name </h4>
                    <p className="productName">apple airpods</p>

                        <div className="productSelectItem">
                            <label>in stock</label>
                            <select className="productSelect" name="stock" id="stock">
                                <option value="yes">yes</option>
                                <option value="no">no</option>
                            </select>
                        </div>
                        
                        <div className="productSelectItem">
                            <label>active</label>
                            <select className="productSelect" name="active" id="active">
                                <option value="yes">yes</option>
                                <option value="no">no</option>
                            </select>
                        </div>
                        

                    </div>
                    <div className="productUpdateRight">
                      <div className="productrUpdateLoad">
                           <img className="productUpdateImage" src="https://geodash.gov.bd/uploaded/avatars/netai/resized/240/netai.jpg" alt=""/>
                           <label htmlFor="file"><CloudUploadIcon  className="userCreateIcon"/></label>
                           <input type="file" id="file" style={{display:"none"}}/>
                      </div>  
                       <button className="productCreateButton">update</button>
                    </div>
                  </form>
                </div>
            </div>
            
         

        </div>
        
    );
};

export default Product;