import { span } from "@material-ui/core"
import "./newmembers.css";
import ArtTrackIcon from '@material-ui/icons/ArtTrack';

const NewMembers = () => {
    return (
        <div className="newMembers">
            <h3 className="newMemberTitle">new join members</h3>
            <ul className="singleMemberList ">

                 <li className="singleMemberListItem">
                    <img className="newmembersImage" alt="" src="https://cdn.pixabay.com/photo/2014/04/02/17/07/user-307993_960_720.png"/>
                    <div className="newMemberIntro">
                        <span className="memberName">Marie Curie</span>
                        <span className="memberNamePosition">software engineer </span>
                    </div> 
                    <span className="newMemberbutton"> 
                    <ArtTrackIcon className="newmemberIcon"/>
                    display
                    </span>
                 </li>
                 <li className="singleMemberListItem">
                    <img className="newmembersImage" alt="" src="http://i.imgur.com/RP1Z4WT.jpg"/>
                    <div className="newMemberIntro">
                        <span className="memberName">Marie Curie</span>
                        <span className="memberNamePosition">software engineer </span>
                    </div> 
                    <span className="newMemberbutton"> 
                    <ArtTrackIcon className="newmemberIcon"/>
                    display
                    </span>
                 </li>
                 <li className="singleMemberListItem">
                    <img className="newmembersImage" alt="" src="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQzyALOcLp4ykOIC4bim8L0xZIvgfLLZEo-mg&usqp=CAU"/>
                    <div className="newMemberIntro">
                        <span className="memberName">Marie Curie</span>
                        <span className="memberNamePosition">software engineer </span>
                    </div> 
                    <span className="newMemberbutton"> 
                    <ArtTrackIcon className="newmemberIcon"/>
                    display
                    </span>
                 </li>
                 <li className="singleMemberListItem">
                    <img className="newmembersImage" alt="" src="https://www.hubspot.com/hubfs/how-to-create-user-accounts-and-profiles.jpeg"/>
                    <div className="newMemberIntro">
                        <span className="memberName">Marie Curie</span>
                        <span className="memberNamePosition">software engineer </span>
                    </div> 
                    <span className="newMemberbutton"> 
                    <ArtTrackIcon className="newmemberIcon"/>
                    display
                    </span>
                 </li>
                 <li className="singleMemberListItem">
                    <img className="newmembersImage" alt="" src="data:image/jpeg;base64,"/>
                    <div className="newMemberIntro">
                        <span className="memberName">Marie Curie</span>
                        <span className="memberNamePosition">software engineer </span>
                    </div> 
                    <span className="newMemberbutton"> 
                    <ArtTrackIcon className="newmemberIcon"/>
                    display
                    </span>
                 </li>
                 <li className="singleMemberListItem">
                    <img className="newmembersImage" alt="" src="http://i.imgur.com/RP1Z4WT.jpg"/>
                    <div className="newMemberIntro">
                        <span className="memberName">Marie Curie</span>
                        <span className="memberNamePosition">software engineer </span>
                    </div> 
                    <span className="newMemberbutton"> 
                    <ArtTrackIcon className="newmemberIcon"/>
                    display
                    </span>
                 </li>
                 <li className="singleMemberListItem">
                    <img className="newmembersImage" alt="" src="https://www.pngitem.com/pimgs/m/0-6243_user-profile-avatar-scalable-vector-graphics-icon-woman.png"/>
                    <div className="newMemberIntro">
                        <span className="memberName">Marie Curie</span>
                        <span className="memberNamePosition">software engineer </span>
                    </div> 
                    <span className="newMemberbutton"> 
                    <ArtTrackIcon className="newmemberIcon"/>
                    display
                    </span>
                 </li>


            </ul>
            
        </div>
    );
};

export default NewMembers;